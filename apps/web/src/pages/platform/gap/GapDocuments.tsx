...
export default function GapDocuments() {
  ...
  useEffect(() => {
    // Fetch status options from code master
    api.get('/api/platform/codes?group=DOC_STATUS')
      .then(r => setStatusOptions(r.data?.items || []))
      .catch(() => setStatusOptions([
        { codeValue: '유효', label: '유효' },
        { codeValue: '만료', label: '만료' },
        { codeValue: '갱신필요', label: '갱신필요' },
      ]));

    // For now, use mock docs as there's no dedicated documents endpoint
    setDocs(MOCK_DOCS);
  }, []);

  const statusColor = (s: string) => s === '유효' ? '#22C55E' : s === '만료' ? '#EF4444' : '#F0A500';

  return (
    <div className="p-8" style={{ background: '#070C12', minHeight: '100vh', color: '#F1F5F9' }}>
      <h1 className="text-xl font-bold mb-6">서류 조회</h1>

      {/* Labor contract block warning -- KEEP THIS */}
      ...
      {statusOptions.length > 0 && (
        <div className="mb-4 text-xs" style={{ color: '#64748B' }}>
          서류 상태: {statusOptions.map((o: any) => o.codeValue || o.label).join(', ')}
        </div>
      )}
