        ...
        <div className="space-y-2 mb-6">
          {equips.map(e => (
            <div key={e.equipId} style={{ background:'#0C1520', border:'1px solid #1E293B' }} className="rounded-lg p-5">
              ...
              <div className="grid grid-cols-4 gap-2 text-xs" style={{ color:'#94A3B8' }}><div>제조사: {e.manufacturer}</div><div>제작: {e.mfgYear}</div><div>구동: {e.driveType}</div><div>차대: {e.chassisNo}</div></div>
            </div>
          ))}
        </div>
        {validation && (
      ...
      {tab === 'survey' && (
        <div style={{ background:'#0C1520', border:'1px solid #1E293B' }} className="rounded-lg p-6">
          ...
          <div className="mb-4"><div className="text-xs font-semibold mb-2" style={{ color:'#F0A500' }}>지하매설물 6종</div>
            <div className="grid grid-cols-3 gap-2">{['가스관','통신선','전력선','상수도','하수도','기타'].map(l => (
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">{[['지반종류','토사/암반/혼합'],['지반상태','견고/연약/동결'],['배수상태','양호/불량'],['보강방법','철판설치/지반개량/양질토사']].map(([l,h]) => (
