...
exports.DISCLAIMER = exports.DOOR_PRICE = exports.MISC_PRICE = exports.PIPE_PRICE = exports.PANEL_PRICE = exports.REGION_DB = void 0;
exports.calcBBDeductRate = calcBBDeductRate;
exports.getBaseHwangdae = getBaseHwangdae;
exports.getDustTier = getDustTier;
exports.getFinalHwangdae = getFinalHwangdae;
exports.getGichoLength = getGichoLength;
exports.getStructType = getStructType;
exports.makeDesign = makeDesign;
exports.calcBOM = calcBOM;
exports.calcLabor = calcLabor;
exports.calcTransport = calcTransport;
exports.calcGate = calcGate;
exports.calcEstimate = calcEstimate;
exports.calc8Matrix = calc8Matrix;
exports.getHeightSteps = getHeightSteps;
exports.inferRegion = inferRegion;
exports.validateDeviation = validateDeviation;
...
const BB_PARAMS = {
    ...
    // ── 판넬류 ──
    RPP: { 고재: { init: 0.68, r1: 0.019, fl: 0.04 }, 신재: { init: 0.43, r1: 0.019, r2: 0.010, piv: 12, fl: 0.04 } },
    EGI: { 고재: { init: 0.72, r1: 0.020, fl: 0.05 }, 신재: { init: 0.47, r1: 0.020, r2: 0.010, piv: 12, fl: 0.05 } },
...
function calcBBDeductRate(itemName, grade, months) {
    if (months <= 0)
        return 0;
...
function getPriceGrade(cat, asset) {
    if (cat === 'clamp')
        return '신재'; // 단가는 항상 신재
    if (asset === '전체신재')
        return '신재';
...
// 하위호환
function getGrade(cat, asset) {
    return getPriceGrade(cat, asset);
}
// ══════════════════════════════════════════
// 횡대 기본단수 (★ v2.1: h>=4 조건)
// ══════════════════════════════════════════
function getBaseHwangdae(h, panel) {
    let base = h <= 2 ? 2 : h <= 3 ? 3 : h <= 4 ? 3 : h <= 5 ? 4 : h <= 6 ? 5 : h <= 7 ? 6 : h <= 8 ? 7 : h <= 9 ? 8 : 9;
    if (panel === '스틸' && Math.floor(h) === 4)
        base = 4;
    if (panel === 'EGI' && h > 4)
        base += 1;
    return base;
}
function getDustTier(dustH) {
    if (dustH <= 0)
        return 0;
    if (dustH <= 1.6)
        return 1;
...
function getFinalHwangdae(h, panel, isStd, dustN) {
...
// ══════════════════════════════════════════
function getGichoLength(h, isStd, floor) {
    if (floor === '콘크리트')
        return null;
    const base = { 1: 1.5, 2: 1.5, 3: 1.5, 4: 1.5, 5: 2.0, 6: 2.0, 7: 2.5, 8: 3.0, 9: 3.0, 10: 3.5 };
    const std = { 1: 1.5, 2: 1.5, 3: 2.0, 4: 2.0, 5: 2.5, 6: 3.0, 7: 3.0, 8: 3.5, 9: 3.5, 10: 4.0 };
    const table = isStd ? std : base;
    return table[Math.floor(h)] ?? 2.0;
}
function getStructType(h) {
    if (h <= 3)
        return '비계식';
    if (h <= 4)
        return '비계식';
...
function makeDesign(h, floor, panel, std, dustN = 0) {
...
function calcBOM(len, h, panel, design, dustN) {
    ...
    let panelQty;
    if (panel === 'RPP')
        panelQty = Math.ceil(len / 0.67);
    else if (panel === 'EGI')
        panelQty = Math.ceil(len / 0.53);
    else
        panelQty = Math.ceil(len * h / 0.99);
    // 전용부자재
    let specialName = '';
    let specialQty = 0;
    let specialPrice = 0;
    if (panel === '스틸') {
        ...
        specialName = '후크볼트';
        specialQty = panelQty * hwN * 2;
        specialPrice = exports.MISC_PRICE.후크볼트;
    }
...
// ══════════════════════════════════════════
function calcLabor(len, h, panel, span, isBB, dustH, isHBeam) {
...
// 운반비
// ══════════════════════════════════════════
function calcTransport(dist, len, isBB) {
...
function calcGate(gate, grade, W, _h, mesh) {
    if (!gate || gate === '없음')
        return { total: 0, body: 0, meshAmt: 0, meshQty: 0 };
    if (gate === '홀딩도어') {
        const b = W * exports.DOOR_PRICE.홀딩도어[grade];
        const meshQty = mesh ? W : 0; // 문짝 1M당 1장
        const m = meshQty * exports.DOOR_PRICE.수직포망;
        return { total: b + m, body: b, meshAmt: m, meshQty };
    }
    if (gate === '양개_비계') {
        const b = W * exports.DOOR_PRICE.양개_비계[grade];
        return { total: b, body: b, meshAmt: 0, meshQty: 0 };
    }
    if (gate === '양개_각관') {
        const b = W * exports.DOOR_PRICE.양개_각관[grade];
        return { total: b, body: b, meshAmt: 0, meshQty: 0 };
    }
    return { total: 0, body: 0, meshAmt: 0, meshQty: 0 };
}
function calcEstimate(input, design, opts) {
    const reg = exports.REGION_DB[input.region] ?? exports.REGION_DB['경기남부'];
    ...
    const items = [
        { name: input.panel, qty: bom.panelQty, price: (exports.PANEL_PRICE[input.panel] ?? {})[pg] ?? 5000, bbGrade: bbPG, bbKey: input.panel },
        { name: '주주파이프', qty: bom.juju, price: exports.PIPE_PRICE.주주파이프[pig], bbGrade: bbPiG, bbKey: '주주파이프' },
        { name: '횡대파이프', qty: bom.hwCnt, price: exports.PIPE_PRICE.횡대파이프[pig], bbGrade: bbPiG, bbKey: '횡대파이프' },
        { name: '지주파이프', qty: bom.jiuju, price: exports.PIPE_PRICE.지주파이프[pig], bbGrade: bbPiG, bbKey: '지주파이프' },
        { name: '기초파이프', qty: bom.gichoQty, price: exports.PIPE_PRICE.기초파이프['고재'], bbGrade: '고재', bbKey: '기초파이프' },
        { name: '고정클램프', qty: bom.gojung, price: exports.MISC_PRICE.고정클램프, bbGrade: bbCG, bbKey: '고정클램프' },
        { name: '자동클램프', qty: bom.jadong, price: exports.MISC_PRICE.자동클램프, bbGrade: bbCG, bbKey: '자동클램프' },
        { name: '연결핀', qty: bom.pin, price: exports.MISC_PRICE.연결핀, bbGrade: bbCG, bbKey: '연결핀' },
        { name: '분진망', qty: bom.dustRolls, price: exports.MISC_PRICE.분진망, bbGrade: '신재', bbKey: '분진망' },
...
// 8조합 매트릭스
// ══════════════════════════════════════════
function calc8Matrix(input, floor, opts) {
    const assets = ['전체고재', '전체신재', '판넬만신재', '파이프만신재'];
    const dustN = getDustTier(opts.dustH ?? 0);
    const design = makeDesign(input.h, floor, input.panel, false, dustN);
    const baseOpts = { bbMonths: opts.bbMonths ?? 6, gate: opts.gate ?? '없음', doorGrade: opts.doorGrade ?? '신재', doorW: opts.doorW ?? 4, doorMesh: opts.doorMesh ?? false, dustH: opts.dustH ?? 0 };
    const bbResults = {};
    const sellResults = {};
    for (const a of assets) {
        bbResults[a] = calcEstimate({ ...input, floor, asset: a, contract: '바이백' }, design, baseOpts);
        sellResults[a] = calcEstimate({ ...input, floor, asset: a, contract: '구매' }, design, { ...baseOpts, bbMonths: 0 });
    }
...
// 유틸
// ══════════════════════════════════════════
function getHeightSteps(panel) {
    if (panel === 'EGI')
        return [1, 1.8, 2, 2.4, 3, 4, 5, 6, 7, 8, 9, 10];
...
function inferRegion(addr) {
    ...
    if (/부산/.test(s))
        return '부산';
    if (/강릉|속초|동해|삼척/.test(s))
        return '강원해안';
    if (/강원|춘천|원주/.test(s))
        return '강원내륙';
    if (/서산|태안|당진/.test(s))
        return '충남서해';
    if (/충청|충남|충북|세종|대전/.test(s))
        return '충청';
    if (/전라|광주|순천|여수/.test(s))
        return '전라';
    if (/경상|대구|울산|창원|포항/.test(s))
        return '경상';
    if (/안산|시흥|평택|화성|수원/.test(s))
        return '경기서해안';
    if (/의정부|파주|고양|포천|양주/.test(s))
        return '경기북부';
    if (/인천/.test(s))
        return '인천';
    if (/서울/.test(s))
...
function generateComments(len, asset, contract, gate, trend) {
    ...
    if (len <= 50)
        ...
        c.push('데이터 충분');
    if (len <= 50)
        c.push('소규모: M당 60%+ 높을 수 있음');
    if (len >= 300)
        c.push('대규모: M당 약 3% 절감');
    if (asset?.includes('고재') && contract === '바이백')
        c.push('고재 BB차감률 높음→자재비↓');
    if (gate !== '없음')
        c.push('도어 별도 산출');
    c.push('경비 ±30% 변동 가능');
    return c;
}
exports.DISCLAIMER = '본 견적은 과거 시공 데이터 기반 예상 범위이며 구조설계 도서가 아닙니다.';
