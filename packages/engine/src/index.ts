...
export function getBaseHwangdae(h: number, panel: string): number {
  // ★ 실전형 횡대 확정표 (v2.1 확정본 2.5.3)
  const PRACTICAL_TIER: Record<number, number> = {
    1: 2, 2: 2, 3: 3, 4: 3, 5: 4, 6: 5, 7: 6, 8: 7,
  };
  let base = PRACTICAL_TIER[Math.floor(h)] ?? Math.max(3, Math.floor(h));
  // 스틸 4M은 실전형도 4단
...
// ══════════════════════════════════════════
// 기초파이프 근입깊이 (실전형 = 높이별 고정, 구조형 = 엔진 계산)
// ★ 확정: 실전형은 1~4→1.5, 5~6→2.0, 7→2.5, 8+→3.0
// ★ 구조형은 CalcStructSpec embedDepth 사용 (Premium.tsx에서 덮어씀)
// ══════════════════════════════════════════
export function getGichoLength(h: number, isStd: boolean, floor: string): number | null {
  if (floor === '콘크리트') return null;
  // 실전형: 높이별 고정값 (확정)
  const practical: Record<number,number> = {1:1.5,2:1.5,3:1.5,4:1.5,5:2.0,6:2.0,7:2.5,8:3.0,9:3.0,10:3.0};
  // 구조형: 기본값 (CalcStructSpec이 덮어씀, fallback용)
  const structural: Record<number,number> = {1:1.5,2:1.5,3:1.5,4:1.5,5:2.0,6:2.0,7:2.5,8:3.0,9:3.5,10:4.0};
  const table = isStd ? structural : practical;
  return table[Math.floor(h)] ?? 2.0;
}

// 기초파이프 노출부 (근입 위 지상구간)
// ★ BOM 기초파이프 총길이 = 근입 + 노출
export function getExposedLen(h: number): number {
  if (h <= 4) return 1.0;
  if (h <= 6) return 1.5;
  return 2.0;
}
...
export function makeDesign(h: number, floor: string, panel: string, std: boolean, dustN: number = 0, constructionType?: '자동' | '비계식' | 'H빔식'): Design {
  ...
  return {
    ...
    jiju: isHB ? 'H빔 자립' : '1:1',  // ★ NS기업 표준 = 1:1 고정
    bojo: isHB ? 'H빔 자립' : (std ? '엔진 계산' : '없음'),  // ★ 실전형=보조지주 없음, 구조형=CalcStructSpec 결정
...
export function calcEstimate(input: QuoteInput, design: Design, opts: CalcOpts): EstimateResult {
  ...
  // H빔 길이 = 판넬높이 + 근입깊이 (노출부는 기초파이프가 담당)
  const hbeamLen = input.h + (design.gichoLength || 2);
  // 기초파이프 총길이 = 근입 + 노출 (BOM 규격에 반영)
  const gichoTotalLen = (design.gichoLength || 2) + getExposedLen(input.h);
